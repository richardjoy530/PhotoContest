...
export class GalleryComponent {
  ...
  constructor(private firestore: AngularFirestore, private storage: AngularFireStorage, public authService: AuthService) {
    ...
    var theme = "Plants and Garden" // Need to fix this one day
  ...
  uploadToFireStorage(): void {
    if (this.caption && this.imgSelected) {
      this.uploading = true;
      const filePath = `/entries/${new Date().getTime()}`;
  ...
  addToFirestore(entry: PhotoEntry): void {
    this.photoEntryConnection.add(entry);
  }

  useVote(priority: number, id: string): void {
  ...
  onVoted(priority: number, entry: PhotoEntryID): void {
    const ref = this.firestore.doc(this.authService.config?.theme + '/' + entry.id);
    if (this.authService.currentUser?.uid) {
      ...
      if (!entry.likedPeoples.includes(this.authService.currentUser.uid)) {
        const likedUsers = entry.likedPeoples;
  ...
  onLike(entry: PhotoEntryID): void {
    if (this.authService.currentUser?.uid) {
  ...
  getRemainingVotes(): number {
    let votes = this.authService.userData?.first === '' ? 1 : 0;
  ...
  isVoted(id: string): boolean {
    return this.authService.userData?.first === id ||
      this.authService.userData?.second === id ||
      this.authService.userData?.third === id ? true : false;
  }

  expandImage(entry: PhotoEntryID): void {
